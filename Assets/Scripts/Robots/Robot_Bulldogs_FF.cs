...
public class Robot_Bulldogs_FF : RobotInterface3D
{
    ...
    public override void Update_Robot()
    {
        ...
        if (gamepad1_dpad_down )
        {
            ...
            MoveHinge(bucket, -1f * forearm.spring.targetPosition, 2f * forearm_speed);
        ...
        // A button runs intake
        if (gamepad1_a || (intake_statemachine == intakeStates.on))
        {
            ...
            // If it was a button_a press, make sure to disengage state machine
        ...
        if (gamepad1_y || (intake_statemachine == intakeStates.reverse))
        {
            ...
            JointMotor outmotor = intake_collector.motor;
            if (outmotor.targetVelocity != -0.5f * intake_speed)
            {
                outmotor.targetVelocity = -0.5f * intake_speed;
                intake_collector.motor = outmotor;
                intake_collector.useMotor = true;
        ...
        if( !gamepad1_a && !gamepad1_y && (intake_statemachine == intakeStates.off))
        {
            // Keep it off
            JointMotor outmotor = intake_collector.motor;
