...
public:
  ...
  void updateVescData(float *fetTemps, float *voltages, float *currents,
                      float *motorTemps, int *erpms, float *dutyCycle);
  void updateIMUData(float rotations, float rotationVelocitys, float accelX,
                     float accelY);
  void updateRadioData(int radioStatus, int powerLevel, long invalidPacketCount,
                       int radioChannel);
