...
void Drive(DriveCommand &command) {
  ...
  vesc.Drive(leftPower, rightPower, command.selfRighterPower,
             command.selfRighterDuty);
}

void DriveWeapons(DriveCommand &command) {
  // Serial.println("Driving weapons with powers: " +
  // String(command.frontWeaponPower) + ", " + String(command.backWeaponPower));
  // set status led
  ...
  if ((ignoreMessageID && (msg.timestamp == lastPacketID)) ||
    ...
    if (lastMessage.type == AUTO_DRIVE) {
      ...
      DriveCommand command = DriveToAngle(
          imu.getRotation(), imu.getRotationVelocity(),
          lastAutoCommand.targetAngle, lastAutoCommand.targetAngleVelocity,
          lastAutoCommand.KD_PERCENT, lastAutoCommand.TURN_THRESH_1_DEG,
      ...
      command.frontWeaponPower =
          (float)lastAutoCommand.frontWeaponCurrent10 * 10.0;
      command.backWeaponPower =
          (float)lastAutoCommand.backWeaponCurrent10 * 10.0;

#ifdef DEBUG_AUTO
      Serial.println("imu rotation: " + (String)imu.getRotation());
      Serial.println("imu rotation velocity: " +
                     (String)imu.getRotationVelocity());
      Serial.println("target angle: " + (String)lastAutoCommand.targetAngle);
      Serial.println("Auto drive command translated to: " +
                     (String)command.movement + " " + (String)command.turn);

      // print coefficients
      Serial.println("ANGLE_EXTRAPOLATE_MS: " +
                     (String)lastAutoCommand.ANGLE_EXTRAPOLATE_MS);
      Serial.println("TURN_THRESH_1_DEG: " +
                     (String)lastAutoCommand.TURN_THRESH_1_DEG);
      Serial.println("TURN_THRESH_2_DEG: " +
                     (String)lastAutoCommand.TURN_THRESH_2_DEG);
      Serial.println("MAX_TURN_POWER_PERCENT: " +
                     (String)lastAutoCommand.MAX_TURN_POWER_PERCENT);
      Serial.println("MIN_TURN_POWER_PERCENT: " +
                     (String)lastAutoCommand.MIN_TURN_POWER_PERCENT);
      Serial.println("SCALE_DOWN_MOVEMENT_PERCENT: " +
                     (String)lastAutoCommand.SCALE_DOWN_MOVEMENT_PERCENT);
      ...
      logger.updateRadioCommandData(
          ...
          lastDriveCommand.frontWeaponPower, lastDriveCommand.backWeaponPower);
#ifdef DEBUG_AUTO
      Serial.println("Manual drive command translated to: " +
                     (String)lastDriveCommand.movement + " " +
                     (String)lastDriveCommand.turn);
#endif
...
bool ErrorCheckMessage(DriverStationMessage &msg) {
  ...
  // else if the message is an auto drive command
    ...
    if (msg.autoDrive.movement > 1.1 || msg.autoDrive.movement < -1.1 ||
        msg.autoDrive.targetAngle > 2 * M_PI ||
        msg.autoDrive.targetAngle < -2 * M_PI || msg.autoDrive.KD_PERCENT < 0 ||
        msg.autoDrive.TURN_THRESH_1_DEG < 0 ||
        msg.autoDrive.TURN_THRESH_1_DEG > 360 ||
        msg.autoDrive.TURN_THRESH_2_DEG < 0 ||
        msg.autoDrive.TURN_THRESH_2_DEG > 360 ||
        msg.autoDrive.MAX_TURN_POWER_PERCENT < 0 ||
        msg.autoDrive.MAX_TURN_POWER_PERCENT > 100 ||
        msg.autoDrive.MIN_TURN_POWER_PERCENT < 0 ||
        msg.autoDrive.MIN_TURN_POWER_PERCENT > 100 ||
        msg.autoDrive.SCALE_DOWN_MOVEMENT_PERCENT < 0 ||
        msg.autoDrive.SCALE_DOWN_MOVEMENT_PERCENT > 100) {
      return false;
    ...
    if (isnan(msg.autoDrive.movement) || isnan(msg.autoDrive.targetAngle) ||
        isnan(msg.autoDrive.KD_PERCENT) ||
        isnan(msg.autoDrive.TURN_THRESH_1_DEG) ||
        isnan(msg.autoDrive.TURN_THRESH_2_DEG) ||
        isnan(msg.autoDrive.MAX_TURN_POWER_PERCENT) ||
        isnan(msg.autoDrive.MIN_TURN_POWER_PERCENT) ||
        isnan(msg.autoDrive.SCALE_DOWN_MOVEMENT_PERCENT)) {
      return false;
