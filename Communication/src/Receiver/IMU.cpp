...
IMU::IMU() { _imu_healthy = true; }

bool IMU::isImuHealthy() { return _imu_healthy; }
...
void IMU::Initialize(enum board_placement placement) {
  ...
  while (!initialized) {
    // use i2c addres 0x68 (which is false). If true, 0x69 (this was how the the
    // port for the old boards)
  ...
  ICM_20948_Status_e retval = myICM.setFullScale(
      (ICM_20948_Internal_Acc | ICM_20948_Internal_Gyr), FSS);
...
void IMU::_updateGyro(double deltaTimeMS) {
  ...
  // time until the gyro calibration weighted average is 1/2 of the way to the
  // new value
  ...
  _smoothRotationVelocity =
      _smoothRotationVelocity +
      (kLPFRotVelBeta * (_currRotVelZ - _smoothRotationVelocity));
  // assume this update's velocity was constant at the average of the previous
  ...
  if (abs(avgRotVelZ) < kCalibrateThreshRadsPerSec) {
    ...
    _calibrationRotVelZ =
        (1 - gyroNewWeight) * _calibrationRotVelZ + gyroNewWeight * avgRotVelZ;
  }
...
void IMU::_updateAccelerometer(double deltaTimeMS) {
  // time until the accel calibration weighted average is 1/2 of the way to the
  // new value
  ...
  double rotatedX =
      _currAcceleration.x * cosRotation - _currAcceleration.y * sinRotation;
  double rotatedY =
      _currAcceleration.x * sinRotation + _currAcceleration.y * cosRotation;
  ...
  if (magnitude(avgAccel) < kCalibrateThreshMpss) {
    ...
    _calibrationAccel =
        _calibrationAccel * (1 - accelNewWeight) + avgAccel * accelNewWeight;
    ...
    if (millis() - _lastAccelerateTimeMS > kResetVelocityNoAccelTimeMs) {
      // if the accelerometer has been stationary for a while, reset the
      // velocity
      _velocity = Point{0, 0, 0};
    }
  } else {
    // if the accelerometer is not stationary, update the last time we
    // accelerated
...
void IMU::Update() {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    const bool boards_data_is_recent =
        (current_time - lastPacketTimestamp[i] < kStaleAngleSyncTimeoutMs);
  ...
  if (still_gyros > 2) {
    // If local gyro thinks robot is moving but others think it's still, mark as
    // unhealthy
    if (startedMovingTimestamp != 0 &&
        (current_time - startedMovingTimestamp > kStillThresholdTimeoutMs)) {
      _imu_healthy = false;
      ForceCalibrate();
    }
    // If local gyro also thinks robot is still, mark as healthy
    else if (stoppedMovingTimestamp[_placement] != 0 &&
             (current_time - stoppedMovingTimestamp[_placement]) >
                 kStillThresholdTimeoutMs) {
      _imu_healthy = true;
    }
  }
...
bool IMU::dataReady() { return myICM.dataReady(); }

/**
 * Gets the acceleration of the robot
 */
Point IMU::getAccel() { return _currAcceleration - _calibrationAccel; }

/**
 * Gets the velocity of the robot
 */
Point IMU::getVelocity() { return _velocity; }

/**
 * Gets the rotational velocity of the robot in radians per second
 */
double IMU::getRotationVelocity() { return _currRotVelZ - _calibrationRotVelZ; }

/**
 * Gets the rotation of the robot in radians
 */
double IMU::getRotation() { return _rotation; }
