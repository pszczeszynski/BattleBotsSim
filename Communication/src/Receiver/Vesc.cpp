...
VESC::VESC(CANBUS *can, int l_drive_id, int r_drive_id, int f_weapon_id,
...
void VESC::OnMessage(const CAN_message_t &msg) {
  ...
  if (msg_id == MSG_ID_INPUT_VOLTAGE) {
    ...
    float duty_cycle =
        int16_t((msg.buf[6] << 8) | msg.buf[7]) / DUTY_CYCLE_SCALE;
    float rpm =
    ...
    float motor_temp =
        int16_t((msg.buf[2] << 8 | msg.buf[3])) / TEMP_AND_CURRENT_SCALE;
    float current =
        int16_t(msg.buf[4] << 8 | msg.buf[5]) / TEMP_AND_CURRENT_SCALE;
...
float absolute(float f) { return f > 0 ? f : -f; }
...
void VESC::_SetMotorPower(float power, int motorIndex) {
  ...
  // we need to do this in reverse order because the VESC expects the bytes in
  // reverse order
  for (int i = 0; i < 4; i++) {
...
void VESC::GetFloatCurrents(float *outCurrents) { outCurrents = _currents; }
void VESC::GetFloatVolts(float *outVolts) { outVolts = _volts; }
void VESC::GetIntRPMs(int *outRPMs) { outRPMs = _rpms; }
void VESC::GetFloatFETTemps(float *outFetTemps) { outFetTemps = _fet_temps; }
