...
// *******************************
// ROBOT interface for FRC Shooter, Inertia and ...
// *******************************
...
public class Robot_FRC_shooter_Peter : RobotInterface3D
{
    ...
    public override void Update_Robot()
    {
        ...
        {
            if (audio_manager && !audio_manager.findSound("adjustangle").isPlaying)
            {
        ...
        if (gamepad1_x)
        {
            ...
            if (collectorWheelVisual_Left)
            { collectorWheelVisual_Left.Rotate(0, 0, Time.deltaTime * 180, Space.Self); }

            if (collectorWheelVisual_Right)
            { collectorWheelVisual_Right.Rotate(0, 0, -Time.deltaTime * 180, Space.Self); }
    ...
    private void Inertia_MoveTurretPulleys()
    {
        ...
        foreach (PullyData t in inertia_shooter_pulleys)
        {
            //(Quaternion.Inverse(ref_start_angle) * refpos.localRotation) * start_angle * delta_angle
            Quaternion flywheelAngle = Quaternion.Inverse(turret.transform.rotation) * inertia_shooter_flywheel.transform.rotation;
            if (t.isReversed)
            {
                flywheelAngle.eulerAngles = new Vector3(flywheelAngle.eulerAngles.x, -flywheelAngle.eulerAngles.y, flywheelAngle.eulerAngles.z);
            }

            t.myTransform.localRotation = flywheelAngle;
        }
    ...
    {
        if (!inertia_shooter_flywheel) { return; }
        JointMotor newMotor = inertia_shooter_flywheel.motor;
        newMotor.force = 0.5f;
        newMotor.targetVelocity = -900;
        newMotor.freeSpin = true;
        inertia_shooter_flywheel.motor = newMotor;
    }
