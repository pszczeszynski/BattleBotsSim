...
public class Robot_FRC_shooter : RobotInterface3D
{
    ...
    public override void RobotFixedUpdate()
    {
        ...
        if (turret && myRobotID)
        {
            ...
            delta_angle = (float) MyUtils.AngleWrap(delta_angle);
            if( delta_angle < -8f)
            {
                delta_angle = -8f;
            }
    ...
    public override void Update_Robot()
    {
        ...
        if (gamepad1_dpad_down)
        {
            ...
            if (ball_aimer_cj)
            {
                curr_hinge_pos = MoveHingeCJ(ball_aimer_cj, outhinge_min, aim_tilt_speed * turning_scaler);
            }
            else
            {
        ...
        if (gamepad1_dpad_up)
        {
            ...
            if (ball_aimer_cj)
            {
                curr_hinge_pos = MoveHingeCJ(ball_aimer_cj, outhinge_max, aim_tilt_speed * turning_scaler);
            }
            else
            {
        ...
        if (gamepad1_b) // Seek home position 
        {
            ...
            if (ball_aimer_cj)
            {
                curr_hinge_pos = MoveHingeCJ(ball_aimer_cj, aim_target_pos, aim_tilt_speed);
            }
            else
            {
        ...
        if (intake_statemachine == intakeStates.onNormal)
        {
            ...
            if( cpanel_wheel)
            {
                outmotor = cpanel_wheel.motor;
                outmotor.targetVelocity = cpanel_speed * turning_scaler;
                cpanel_wheel.motor = outmotor;
            }
        }
        if (intake_statemachine == intakeStates.off)
        {
        ...
        if (intake_statemachine == intakeStates.reverse)
        {
            ...
            {
                outmotor = cpanel_wheel.motor;
                outmotor.targetVelocity = -1f * cpanel_speed * turning_scaler;
                cpanel_wheel.motor = outmotor;
            }
        }
    ...
    public void DoAnimations(bool client_mode = false)
    {
        ...
        // Do panel wheel animation
        if(cp_wheel_animation)
        {
            float speed = cpanel_speed * Time.deltaTime * turning_scaler;
            if (intake_statemachine == intakeStates.reverse)
            { speed *= -1f; }
            if (intake_statemachine == intakeStates.off)
            { speed = 0f; }

            cp_wheel_animation.Rotate(0, speed, 0, Space.Self);
        }
