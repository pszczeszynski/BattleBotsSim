...
RobotSimState Orbit::_ExtrapolateOurPos(double seconds_position, double seconds_angle)
{
    ...
    currentState.angle = odoData.GetAngle();
    currentState.velocity = odoData.robotVelocity;
    currentState.angularVelocity = odoData.GetAngleVelocity() * angle_vel_scale;
...
{
    // get the robot angle
    double robotAngle = RobotController::GetInstance().odometry.Robot().GetAngle();
...
cv::Point2f Orbit::_GetOrbitFollowPoint(bool circleDirection, double& outCost, bool draw)
{
    ...
    cv::Point2f opponentWeaponPosEx = GetWeaponPos(opponentExState.robotPosition, opponentData.GetAngle(), OPPONENT_WEAPON_OFFSET);
    ...
    double orbitRadius = _CalculateOrbitRadius(angleWeaponToUs, exState.position, opponentExState.robotPosition, opponentExState.GetAngle(), gamepad, circleDirection);
    double dangerLevel = _GetDangerLevel(angleWeaponToUs, opponentExState.GetAngle(), circleDirection);
    ...
    if (draw)
    {
        ...
        safe_arrow(drawingImage, opponentExState.robotPosition,
                        opponentExState.robotPosition + cv::Point2f(cos(opponentExState.GetAngle()) * 50,
                                                               sin(opponentExState.GetAngle()) * 50),
    ...
    // add turning score
    double angleToTarget = atan2(targetPoint.y - odoData.robotPosition.y, targetPoint.x - odoData.robotPosition.x);
    double angleDiff = angle_wrap(angleToTarget - odoData.GetAngle() + (LEAD_WITH_BAR ? 0 : M_PI));
    double angleCost = abs(angleDiff) / (180 * TO_RAD);

    double angleToButt = angle_wrap(opponentExState.GetAngle() + M_PI);
    // 0 -> 2pi
    double angleFromUsToButt = angle_wrap(angleToButt - angleWeaponToUs - M_PI) + M_PI;

    // draw an arrow from the opponent to the weapon
    safe_arrow(drawingImage, opponentExState.robotPosition,
                    opponentExState.robotPosition + cv::Point2f(cos(opponentExState.GetAngle() + M_PI) * 50,
                                                           sin(opponentExState.GetAngle() + M_PI) * 50),
