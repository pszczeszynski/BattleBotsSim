...
void Vision::runPipeline()
{
    ...
    // 1. find the opponent
    cv::Point2f opponent_new = findOpponent();
    // 2. crop the image around the opponent
    unsigned int cropSize = 200;
    cv::Rect opponentRect = cv::Rect(opponent_new.x - cropSize / 2, opponent_new.y - cropSize / 2, cropSize, cropSize);
    cv::Mat croppedFrame = frame(opponentRect);
    ...
    // 2. track them
    opponentOpticalFlow.PerformMotionDetection(frame, drawing_image);
}
...
cv::Point2f Vision::findOpponent()
{
    ...
    // looks for motion in the image, and returns the center of the largest motion blob
    cv::Mat frame = overheadCam.getFrame();

    // If this is the first frame we have nothing to compare with, so just return a default point
    ...
    // check if there is any motion in the image
    if (cv::countNonZero(grayDiff) == 0)
    {
        return center;
    }
    ...
    cv::blur(thresholdImg, thresholdImg, cv::Size(7,7));
    ...
    float minArea = 250;
    for (int y = 0; y < thresholdImg.rows; y+= 5)
    {
        for (int x = 0; x < thresholdImg.cols; x+= 5)
        {
    ...
    for (const cv::Rect &rect : potentialRobots)
    {
        ...
        robotCenters.push_back(averageWhitePixel);
    }

    updateRobotTrackers(robotCenters, frame);
...
void Vision::updateRobotTrackers(std::vector<cv::Point2f> centers, cv::Mat& frame)
{
    // minimize cost of updating
    if (centers.empty()) return;

    double COST_THRESHOLD = 100;
    ...
    if (centers.size() == 1)
    {
        ...
        if (cost1 < cost2 && cost1 < COST_THRESHOLD)
        {
            robotTrackers[0].update(centers[0], frame);
        }
        else if (cost2 < COST_THRESHOLD)
        {
    ...
    {
        if (cost00 < COST_THRESHOLD) robotTrackers[0].update(centers[0], frame);
        if (cost11 < COST_THRESHOLD) robotTrackers[1].update(centers[1], frame);
    }
    else
    {
        if (cost01 < COST_THRESHOLD) robotTrackers[0].update(centers[1], frame);
        if (cost10 < COST_THRESHOLD) robotTrackers[1].update(centers[0], frame);
    }
}