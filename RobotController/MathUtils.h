...
class Point2f {
 public:
  ...
  Point2f operator*(const double& d) const { return Point2f(x * d, y * d); }
...
class Angle {
 public:
  ...
  Angle operator*(const double& d) const { return Angle(value_ * d); }

  Angle operator/(const double& d) const { return Angle(value_ / d, false); }

  // Conversion operator to double
  operator double() const { return value_; }

  double degrees() const { return value_ * TO_DEG; }
