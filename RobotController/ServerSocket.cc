...
int ServerSocket::setup_receiving_socket() {
  ...
  if (bind(listenSocket, (SOCKADDR *)&serverAddress, sizeof(serverAddress)) ==
      SOCKET_ERROR) {
  ...
  while (true) {
    ...
    if (numBytesReceived == SOCKET_ERROR) {
      ...
      if (err == WSAEWOULDBLOCK) {
        ...
        // Error occurred
        std::cerr << "recvfrom on port " << port
                  << " failed with error: " << err << std::endl;
        if (outError != nullptr) {
          *outError = err;
          return "";
        }
...
void ServerSocket::reply_to_last_sender(std::string data) {
  if ((listenSocket == INVALID_SOCKET) || (last_sender_addr_len == 0) ||
      (data.length() > 65400))  // leave room for unknown overhead
  {
    return;
  }

  int iSendResult = sendto(listenSocket, data.c_str(), data.length(), 0,
                           (SOCKADDR *)&last_sender_addr, last_sender_addr_len);
