            ...
            while (c_retryTime.getElapsedTime() < PRIMARY_TIMEOUT)
            {
                for (int i = 0; i < devices_opened; i++)
                {
                    ...
                    if (num >= sizeof(RobotMessage))
                    {
                        ...
                        RobotMessage msg =
                            *reinterpret_cast<RobotMessage *>(recvbuf);
                ...
                if (messageMutex->try_lock())
                {
                    ...
                    if (delay)
                    {
                        auto elapsed =
                            ...
                            _startTime;
                        uint32_t start = std::chrono::duration_cast<
                    ...
                    if (!dev->SendAsync(buf, HID_BUFFER_SIZE))
                        _radio_reinit = true;
                    *newMessage = false;
                }
            }
    ...
    Clock *lastReceivedTimer, std::mutex *radioStatsMutex)
{
    ...
    while (true)
    {
        ...
        if (!_radio_reinit && dev->IsOpen())
        {
            ...
            {
                if (!dev->RecvAsync(HID_BUFFER_SIZE))
                    _radio_reinit = true;
...
void RobotLinkReal::RadioThreadFunction(void)
{
    ...
    std::thread primayRadioSendThread(
        &RobotLinkReal::RadioThreadSendFunction, this, &_radios[0],
        &_requestSend, &_messageToSend, &_sendMessageMutex, false);
    ...
    while (true)
    {
        ...
        {
            devices_opened =
                RawHID_Open(_radios, 2, 0x16C0, 0x0486, 0xFFAB, 0x0200);
            std::this_thread::sleep_for(
                std::chrono::milliseconds(USB_RETRY_TIME));
        }
        _transmitterConnected = true;
...
void RobotLinkReal::ChooseBestChannel(DriverStationMessage &msg)
{
    ...
    {
        cv::Mat &drawingImage =
            RobotController::GetInstance().GetDrawingImage();
        ...
        else if (switchedSecondary)
        {
            cv::putText(drawingImage,
                        "Switched Secondary to " +
                            std::to_string(SECONDARY_RADIO_CHANNEL),
                        cv::Point(WIDTH / 2 - 200, 50),
                        cv::FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(0, 0, 255), 2);
        }
    }
    ...
    if (_transmitterConnected && _secondaryTransmitterConnected)
    {
        ...
        nextChannel = TEENSY_RADIO_1 + TEENSY_RADIO_2 + TEENSY_RADIO_3 -
                      RADIO_CHANNEL - SECONDARY_RADIO_CHANNEL;
    }
    else
    {
...
std::vector<RobotMessage> RobotLinkSim::_ReceiveImpl()
{
    ...
    else
    {
        ...
        ret.canData.motorERPM[2] =
            (int)abs(message.spinner_1_RPM * RPM_TO_ERPM / ERPM_FIELD_SCALAR) *
            9;
        ret.canData.motorERPM[3] =
            (int)abs(message.spinner_2_RPM * RPM_TO_ERPM / ERPM_FIELD_SCALAR) *
            9;
    ...
    opponentVelSim = (opponentPosSim - lastOpponentPosSim) /
                     lastReceiveClock.getElapsedTime();
