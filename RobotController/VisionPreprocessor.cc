...
VisionPreprocessor::VisionPreprocessor() {
  ...
  // Initialize our dstPointsList. Also populate srcPointList so we dont have to
  // do it later
  for (float y = 0; y <= gridSize - 1; y += 1.0) {
    for (float x = 0; x <= gridSize - 1; x += 1.0) {
      std::vector<cv::Point2f> innerRec;
      innerRec.push_back(
          cv::Point2f(x * WIDTH / gridSize, y * HEIGHT / gridSize));
      innerRec.push_back(
          cv::Point2f((x + 1) * WIDTH / gridSize, y * HEIGHT / gridSize));
      innerRec.push_back(
          cv::Point2f((x + 1) * WIDTH / gridSize, (y + 1) * HEIGHT / gridSize));
      innerRec.push_back(
          cv::Point2f(x * WIDTH / gridSize, (y + 1) * HEIGHT / gridSize));
...
void VisionPreprocessor::Preprocess(cv::Mat& frame, cv::Mat& dst) {
  ...
  warpPerspective(outputImage, dst, _transformationMatrix,
...
void VisionPreprocessor::_StabalizeImage(cv::Mat& frame, cv::Mat& dst) {
  ...
  // std::cout << "track feature time: " << c.getElapsedTime() * 1000 <<
  // std::endl;
  ...
  // std::cout << "warp affine time: " << c.getElapsedTime() * 1000 <<
  // std::endl; c.markStart();
...
cv::Point2f VisionPreprocessor::_TrackFeature(cv::Mat& prevFrame,
                                              cv::Mat& frame, cv::Rect& roi) {
  // crop the frame
  ...
  for (currTranslation.y = -SEARCH_WINDOW; currTranslation.y <= SEARCH_WINDOW;
       currTranslation.y++) {
    for (currTranslation.x = -SEARCH_WINDOW; currTranslation.x <= SEARCH_WINDOW;
         currTranslation.x++) {
...
void VisionPreprocessor::_generateCameraParameters(
    float scaler_fl, float scaler_intensity, float scaler_y, cv::Size imageSize,
  ...
  K = cv::Matx33d(fx, 0, cx, 0, fy, cy, 0, 0, 1);

  // Assume the distortion coefficients (k1, k2, p1, p2) are proportional to the
  // scaler
...
void VisionPreprocessor::ComputePMFisheyePointList(cv::Mat& image) {
  ...
  if (CameraWidget::tuningMode) {
    for (size_t i = 0; i < srcPointsList.size(); i++) {
      cv::line(image, srcPointsList[i][0], srcPointsList[i][1],
               cv::Scalar(0, 255, 0), 2, cv::LINE_AA);
      cv::line(image, srcPointsList[i][1], srcPointsList[i][2],
               cv::Scalar(0, 255, 0), 2, cv::LINE_AA);
      cv::line(image, srcPointsList[i][2], srcPointsList[i][3],
               cv::Scalar(0, 255, 0), 2, cv::LINE_AA);
      cv::line(image, srcPointsList[i][3], srcPointsList[i][0],
               cv::Scalar(0, 255, 0), 2, cv::LINE_AA);
    }
  }
}
...
cv::Point2f VisionPreprocessor::GetPMFisheyeStartPoint(cv::Point2f& point) {
  ...
  // double disRadius = 1000/FISHEYE_FL *
  // atan(2*regRadius*tan(FISHEYE_FL/2000));
  double disRadius =
      4.0 * FISHEYE_FL * sin(atan(regRadius / FISHEYE_FL / 2.0) / 2.0);
  return (center +
          FISHEYE_SCALE / 10.0f * disRadius / regRadius * (point - center));
}

// Poor-Mans Fish-eye
// Ok so this Poor-Mans fish-eye removal takes > 20ms single-threaded, but it
// can be threaded as opposed to a single remap call
void VisionPreprocessor::DoPMFishEye(cv::Mat& inImage, cv::Mat& outImage) {
  ...
  for (size_t i = 0; i < srcPointsList.size(); ++i) {
    ++processes_run;
    auto boundFunction =
        std::bind(&VisionPreprocessor::PMFishEyeCoreThread, this, i,
                  std::ref(inImage), std::ref(dst), std::ref(processes_done),
                  std::ref(_cvFisheye), std::ref(_mutexFisheye));

    ThreadPool::myThreads.enqueue(boundFunction);
  }

  {
    std::unique_lock<std::mutex> locker(_mutexFisheye);
    while (processes_done < processes_run) {
      if (_cvFisheye.wait_for(locker, std::chrono::milliseconds(200)) ==
...
void VisionPreprocessor::PMFishEyeCoreThread(
    size_t i, cv::Mat& inImage, cv::Mat& dst, int& doneInt,
    std::condition_variable_any& doneCV, std::mutex& mutex) {
  ...
  // Copy the warped image into the destination image using the mask to restrict
  // the copy operation to the transformed trapezoid
...
#endif
std::vector<cv::Point> VisionPreprocessor::convertPoints(
    const std::vector<cv::Point2f>& points2f) {
  std::vector<cv::Point> points;
  for (const auto& point2f : points2f) {
    points.push_back(cv::Point(static_cast<int>(roundf(point2f.x)),
                               static_cast<int>(roundf(point2f.y))));
  }
  return points;
}