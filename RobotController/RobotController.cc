...
#endif
{
  ...
  _videoWriter = cv::VideoWriter{_logDirectory + "/output0.avi",
...
void RobotController::DumpVideo() {
  ...
  _videoWriter.open(_logger.GetLogDirectory() + "/output" +
                        std::to_string(_video_index) + ".avi",
...
void drawAndDisplayValue(cv::Mat& image, double value, double xPosition,
                         cv::Scalar color) {
  value *= -1;
  ...
  cv::putText(image, text,
              cv::Point(xPosition, startY + rectHeight + textSize.height + 5),
              cv::FONT_HERSHEY_SIMPLEX, 0.5, color, 1);
}

Gamepad& RobotController::GetGamepad() { return gamepad; }

Gamepad& RobotController::GetGamepad2() { return gamepad2; }
...
DriverStationMessage RobotController::ManualMode() {
  ...
  float selfRighterPower =
      (int)gamepad.GetDpadDown() - (int)gamepad.GetDpadUp();
  selfRighterPower += InputState::GetInstance().IsKeyDown(ImGuiKey_J) -
                      InputState::GetInstance().IsKeyDown(ImGuiKey_U);
  selfRighterPower +=
      (int)gamepad2.GetRightBumper() - (int)gamepad2.GetLeftBumper();
  // control with keyboard if enabled
  if (WASD_ENABLED) {
    response.movement += (int)InputState::GetInstance().IsKeyDown(ImGuiKey_W) -
                         (int)InputState::GetInstance().IsKeyDown(ImGuiKey_S);
    response.turn += (int)InputState::GetInstance().IsKeyDown(ImGuiKey_A) -
...
void RobotController::StartForceOrbit() { _guiOrbit = true; }

void RobotController::StopForceOrbit() { _guiOrbit = false; }

void RobotController::StartForceKill() { _guiKill = true; }

void RobotController::StopForceKill() { _guiKill = false; }
...
DriverStationMessage RobotController::RobotLogic() {
  ...
  // if (_orbiting)
  // {
  //     cv::putText(drawingImage, "Orbit", cv::Point(10, 50),
  //     cv::FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(0, 255, 0), 2);
  // }
  // else if (_killing)
  // {
  //     cv::putText(drawingImage, "Killing", cv::Point(10, 50),
...
IRobotLink& RobotController::GetRobotLink() { return robotLink; }

// Returns the image to do draw overlay info on
cv::Mat& RobotController::GetDrawingImage() { return drawingImage; }
