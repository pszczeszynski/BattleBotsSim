...
float RobotTracker::deltaAngleSweep = 15; // Delta +/- angle to sweep
float RobotTracker::deltaAngleStep = 1; // Number of degrees to step angle sweep
...
MultiThreadCleanup::MultiThreadCleanup(int& count_to_increment, std::mutex& mutex, std::condition_variable_any& cv) : 
    count_to_increment_(count_to_increment), mutex_(mutex), cv_(cv) {};
...
{
    if( point.x < roi.x ) { point.x = roi.x;}
    if( point.y < roi.y) { point.y = roi.y;}
    if( point.x > roi.x+roi.width) { point.x = roi.x + roi.width;}
    if( point.y > roi.y + roi.height) { point.y = roi.y + roi.height;}
...
cv::Rect MoveRectToBeInside(cv::Rect recttofix, cv::Mat& mat_to_fit)
{
    ...
    // Fix x
    if( recttofix.x < 0) { recttofix.x = 0;}
    ...
    // Fix y
    if( recttofix.y < 0) { recttofix.y = 0;}
...
double RobotTracker::FindNewPosAndRotUsingMatchTemplate( cv::Mat& currFrame,  cv::Mat& foreground, cv::Rect& fgFoundBBox,  Vector2& newRot)
{
    ...
    for( float currAngle =-1*deltaAngleSweep; currAngle < deltaAngleSweep; currAngle +=  currAngleIncrement )
    {
        ...
        else
        {
            ...
            myThreads.enqueue([this, &fgFoundBBox, &matchingMat, currAngle, currAngleStop] { this->matchTemplateThread(fgFoundBBox, matchingMat, currAngle, currAngleStop); });
            processes_run++;
        }          
    }
