...
TrackedBBox::TrackedBBox(RobotTracker *tracker) {
  bbox = tracker->bbox;
  velocity = cv::Point2f(tracker->velForMovementDetection.x,
                         tracker->velForMovementDetection.y);
  is_active = true;
...
float FindClosestValidTrackedBox(const std::vector<TrackedBBox> &allBBoxes,
                                 const cv::Point2f &point,
                                 TrackedBBox &bestBBox, int &indexFound,
                                 bool onlyCleared) {
  ...
  for (int index = 0; index < allBBoxes.size(); index++) {
    // Skip if not valid
    if (!allBBoxes[index].is_active) {
      continue;
    }
    if (onlyCleared && !allBBoxes[index].is_clear) {
      continue;
    }
...
std::string HeuristicOdometry::statusstring =
    "Stopped.";  // Static status string to show in the UI

HeuristicOdometry::HeuristicOdometry(ICameraReceiver *videoSource)
...
void HeuristicOdometry::UpdateSettings() {
  ...
  fg_mask_blur_size =
      cv::Size(HEU_FOREGROUND_BLURSIZE, HEU_FOREGROUND_BLURSIZE);
  ...
  RobotTracker::moveTowardsCenter =
      (float)HEU_POSITION_TO_CENTER_SPEED / 100.0f;
  RobotTracker::robotVelocitySmoothing =
      ((float)HEU_VELOCITY_AVERAGING) / 100.0f;
...
bool HeuristicOdometry::IsBackgroundStable() {
  return (heuStateMachine != CMSM_WAIT_BRIGHTNESS) &&
         (heuStateMachine != CMSM_LOADBACK);
}
...
void HeuristicOdometry::_ProcessNewFrame(cv::Mat currFrame, double frameTime) {
  ...
  if (enable_camera_antishake) {
    ...
    // Crop the image
    cv::Rect cropArea(crop_x, crop_y, newFrame.cols - 2 * crop_x,
                      newFrame.rows - 2 * crop_y);
  ...
  cv::Rect cropparea(x_offset, y_offset, newFrame.cols - 2 * crop_x,
                     newFrame.rows - 2 * crop_y);
  cv::Mat correctedFrame =
      (enable_camera_antishake) ? newFrame(cropparea) : newFrame;
  ...
  if (load_background) {
    LoadBackground(newFrame,
                   loadBackgroundsPath +
                       "/savedBackground.jpg");  // Use savebackground path
    load_background = false;
  }

  bool matchstart_was_run = false;

  cv::Rect left_start_rect(STARTING_LEFT_TL_x, STARTING_LEFT_TL_y,
                           (STARTING_LEFT_BR_x - STARTING_LEFT_TL_x),
                           (STARTING_LEFT_BR_y - STARTING_LEFT_TL_y));
  cv::Rect right_start_rect(STARTING_RIGHT_TL_x, STARTING_RIGHT_TL_y,
                            (STARTING_RIGHT_BR_x - STARTING_RIGHT_TL_x),
                            (STARTING_RIGHT_BR_y - STARTING_RIGHT_TL_y));
  float brightness_correction = 0;

  // Cache published position state BEFORE taking _mutexAllBBoxes to avoid
  // deadlock
  bool usHasPos = false;
  ...
  switch (heuStateMachine) {
    ...
    case CMSM_WAIT_BRIGHTNESS:
      ...
      if (brightness_correction < start_brightness_ok_threshold ||
        ...
        char buffer[32];
        std::snprintf(buffer, sizeof(buffer), "Corr = %.2f",
      ...
      if (currTime - brightness_stable_start_time >=
          start_brightness_soak_period) {
        heuStateMachine = CMSM_WAIT_BRIGHTNESS2;
        brightness_stable_start_time =
            currTime;  // Reset soak period but to soak2 now
    ...
    case CMSM_START_WAIT_ROBOT_CLEAR:
      ...
      if (!themHasPos && (opponentRobotTracker != nullptr)) {
        if (IsClearOfArea(opponentRobotTracker, left_start_rect) &&
    ...
    case CMSM_DETECT_MOVING_TRACKERS:
      // During this period any trackers that appear should be cleared only if
      // they move past their starting area If both us and opponent robots are
      // detected, all remaining trackers are merged into background The fusion
      // algorithm will try lock robots, once they are both locked we will clear
      // all remaining trackers
  ...
  // This initializes the background with currFrame but copies over the saved
  // background left/right regions
  if (match_start_bg_init) {
    // First override all variables that matchstatbackground algorithm uses
    // different OVerride fg_threshold, blurr size nad blur count
    fg_threshold = HEU_FOREGROUND_THRESHOLD_INIT;
    blurCount = HEU_BLUR_COUNT_INIT;
    preMaskBlurSize =
        cv::Size(HEU_BACKGROUND_BLURSIZE_INIT, HEU_BACKGROUND_BLURSIZE_INIT);
  ...
  // Correct Brightness (assumes shake doesn't significantly affect it on
  // average)
  ...
  if (save_to_video_match_debug ||
      save_to_video_output)  // Remeber that we are saving video
  {
  ...
  if (save_background_to_files && (currTime - timeTrackForBGSave > 1.0)) {
    DumpBackground("background_" + std::to_string((int)currTime),
                   dumpBackgroundsPath);
  ...
  // From here on maniuplation of all_bboxes and/or tracker (core tracking data)
  // is allowed
  std::unique_lock<std::mutex> locker(_mutexAllBBoxes);
  ...
  currFrameColor =
      cv::Mat::zeros(correctedFrame.size(),
                     CV_8UC3);  // Create a black image for color display
  ...
  // If we have identified both robots during robot clear stage, merge
  ...
  if (((heuStateMachine == CMSM_START_WAIT_ROBOT_CLEAR) ||
    ...
    healBackground(correctedFrame,
                   true);  // Merge all non-robot areas into background
  } else {
    // Heal background
    healBackground(correctedFrame);
  }
  ...
  if (show_bg_mat) {
    ...
    if (!backgroundOverlay.empty() &&
        (backgroundOverlay.size() == currBackground.size())) {
      try {
  ...
  _PublishIfValid(opponentRobotTracker, /*isOpponent=*/true, frameTime,
                  _themState);
  locker.unlock();
  ...
  if (robotData.pos.has_value()) {
    safe_circle(currFrameColor, robotData.pos.value().position, 10,
                cv::Scalar(0, 255, 100), 2, cv::LINE_AA, 0);
  }
  ...
  if (show_track_mat || save_video_enabled && save_to_video_output) {
    std::string debugString =
        RobotController::GetInstance().odometry.GetDebugString();
  ...
  if (clock_outer.getElapsedTime() > 1.0) {
    ...
    for (int i = 0; i < timing_list.size(); i++) {
      printText(timing_text[i] + std::to_string(timing_list[i] * 1000.0) + "ms",
                statsmat, y_offset);
      y_offset += 20;
    }
...
void HeuristicOdometry::AddDebugStringToFrame(cv::Mat &frame,
                                              std::string debugString) {
  int y_offset = 20;
...
void HeuristicOdometry::_UpdateOdometry(OdometryData &data,
                                        OdometryData &oldData,
                                        RobotTracker *tracker,
                                        double timestamp) {
  // DO NOT CALL (old architecture) - This method is deprecated and should never
  // be used. It was part of the old per-frame update model that published
  // invalid/dummy data. Use _PublishIfValid instead for the publish-only model.
  assert(
      false &&
      "_UpdateOdometry should never be called - use _PublishIfValid instead");
  ...
  if (tracker == nullptr) {
    ...
    if (oldData.angle.has_value()) {
      data.angle =
          AngleData(oldData.angle.value().angle, 0, oldData.angle.value().time);
    } else {
      data.angle.reset();
    }
    return;
  ...
  data.pos =
      PositionData(tracker->position.Point2f(), tracker->avgVelocity.Point2f(),
                   tracker->bbox, timestamp);
  ...
  // Make sure deltaTime isn't very small (e.g. we just updated and itll cause a
  // discontinuity)
  double deltaTime =
      timestamp - (oldData.pos.has_value() ? oldData.pos.value().time : 0);
  if ((deltaTime > 1.0f / 250.0f) && (deltaTime < angleVelocityTimeConstant)) {
    newAngleVelocity =
        newAngleVelocity * (1.0 - deltaTime / angleVelocityTimeConstant) +
        1.0 / angleVelocityTimeConstant *
            (newAngle - oldData.angle.value().angle);
...
void HeuristicOdometry::_PublishIfValid(RobotTracker *tracker, bool isOpponent,
                                        double timestamp,
  ...
  if (state.last_angle.has_value()) {
    ...
    if ((deltaTime > 1.0f / 250.0f) &&
        (deltaTime < angleVelocityTimeConstant)) {
      newAngleVelocity = state.last_angle.value().velocity *
                             (1.0 - deltaTime / angleVelocityTimeConstant) +
                         1.0 / angleVelocityTimeConstant *
                             (newAngle - state.last_angle.value().angle);
    } else if (deltaTime > 1.0f / 250.0f) {
      newAngleVelocity =
          (newAngle - state.last_angle.value().angle) / deltaTime;
    } else {
      newAngleVelocity =
...
void HeuristicOdometry::SetPosition(const PositionData &newPos, bool opponentRobot, bool skipMutexLock) 
{
  ...
  RobotTracker *tracker =
      (opponentRobot) ? opponentRobotTracker : ourRobotTracker;
...
void HeuristicOdometry::ForcePosition(const PositionData &newPos,bool opponentRobot) 
{
  ...
  RobotTracker *&firstbot =
      (opponentRobot) ? opponentRobotTracker
                      : ourRobotTracker;  // Guaranteed not to be nullpts
  ...
  if (firstbot->IsPointInside(newPosPt) && !firstbot->IsTrackerCombined()) {
    scenario = 1;
  } else if (firstbot->IsPointInside(newPosPt) &&
             firstbot->IsTrackerCombined()) {
    scenario = 2;
...
void HeuristicOdometry::_StartCalled() { cvWindows.clear(); }
...
void HeuristicOdometry::TrackRobots(cv::Mat &croppedFrame,
  ...
  //
  // Step 2: Let all the tracked items process the bboxes. If the bbox is asked
  // from multiple robots, then treat it like overlly big boxes
  ...
  for (auto currItem : _allRobotTrackers) {
    ...
    if (!useMultithreading) {
      currItem->ProcessNewFrame(
          currTime, foreground, croppedFrame, fg_mask, processes_done,
      ...
      auto boundFunction = std::bind(
          &RobotTracker::ProcessNewFrame, currItem, currTime,
          std::ref(foreground), std::ref(croppedFrame), std::ref(fg_mask),
          std::ref(processes_done), std::ref(conditionVarTrackRobots),
          std::ref(_mutexTrackData),
          std::ref(
              (save_video_enabled && save_to_video_match_debug && i == itarget)
                  ? channels[1]
                  : nullMat),
  ...
  while (processes_done < processes_run) {
    if (conditionVarTrackRobots.wait_for(_mutexTrackData,
  ...
  for (auto currIter = _allRobotTrackers.begin();
       currIter != _allRobotTrackers.end();) {
    // Delete items that haven't been tracked for a while
    if ((deleteForNoTrackingCount > 0) &&
        ((*currIter)->numFramesNotTracked > deleteForNoTrackingCount)) {
      ...
      safe_circle(frameToDisplay, (*currIter)->GetCenter(), 10, colorRectangle,
                  3);
      cv::Point rotationdir((*currIter)->rotation.x * 50.0,
                            (*currIter)->rotation.y * 50.0);
      cv::Point centerBBox = (*currIter)->bbox.tl();
      centerBBox.x += (*currIter)->bbox.width / 2;
      centerBBox.y += (*currIter)->bbox.height / 2;

      cv::Point avgVelocityPoint((*currIter)->avgVelocity.x / 3.0,
                                 (*currIter)->avgVelocity.y / 3.0);

      // Draw avg velocity
      //  cv::line(frameToDisplay, centerBBox, centerBBox + avgVelocityPoint,
      //  cv::Scalar(0,10,150), 3 );

      // Draw rotation
      cv::line(frameToDisplay, centerBBox, centerBBox + rotationdir,
               cv::Scalar(0, 50, 250), 2);
      ...
      ss << "Bbox w,h=" << (*currIter)->bbox.width << ","
         << (*currIter)->bbox.height;
      ...
      ss << "\n"
         ...
         << ")";
      double angle =
          rad2deg(atan2((*currIter)->rotation.y, (*currIter)->rotation.x));
      ss << "\n" << "Rotation=" << angle;

      printText(ss.str(), frameToDisplay, (*currIter)->bbox.y + 20 * line++,
                (*currIter)->bbox.x + (*currIter)->bbox.width);

      // printText("Bbox w,h=" + std::to_string((*currIter)->bbox.width) + "," +
      // std::to_string((*currIter)->bbox.height),
      //           frameToDisplay, (*currIter)->bbox.y + 20 * line++,
      //           (*currIter)->bbox.x + (*currIter)->bbox.width);
      // printText("Velocity=" + std::to_string((*currIter)->avgVelocity.mag()),
      //           frameToDisplay, (*currIter)->bbox.y + 20 * line++,
      //           (*currIter)->bbox.x + (*currIter)->bbox.width);
      // printText("velForMove=" +
      // std::to_string((*currIter)->velForMovementDetection.mag()),
      //           frameToDisplay, (*currIter)->bbox.y + 20 * line++,
      //           (*currIter)->bbox.x + (*currIter)->bbox.width);
      //  printText("noMoving=" + std::to_string((*currIter)->timeNotMoving),
      //      frameToDisplay, (*currIter)->bbox.y+20*line++,
      //      (*currIter)->bbox.x+ (*currIter)->bbox.width);
      //  printText("noTracking=" +
      //  std::to_string((*currIter)->numFramesNotTracked),
      //      frameToDisplay, (*currIter)->bbox.y+20*line++,
      //      (*currIter)->bbox.x+ (*currIter)->bbox.width);
      // printText("delta=(" + std::to_string((*currIter)->delta.x) + "," +
      // std::to_string((*currIter)->delta.y) + ")",
      //           frameToDisplay, (*currIter)->bbox.y + 20 * line++,
      //           (*currIter)->bbox.x + (*currIter)->bbox.width);
      // double angle = rad2deg(atan2((*currIter)->rotation.y,
      // (*currIter)->rotation.x)); printText("Rotation=" +
      // std::to_string(angle),
      //           frameToDisplay, (*currIter)->bbox.y + 20 * line++,
      //           (*currIter)->bbox.x + (*currIter)->bbox.width);
...
void HeuristicOdometry::_allRobotTrackersClear() {
  for (auto currIter = _allRobotTrackers.begin();
       currIter != _allRobotTrackers.end(); ++currIter) {
    _deleteTracker(*currIter);
  }
...
void HeuristicOdometry::LoadBackground(cv::Mat &currFrame,
  ...
  if (regularBackground.empty() ||
    ...
    cv::Rect cropparea(crop_x, crop_y, regularBackground.cols - 2 * crop_x,
                       regularBackground.rows - 2 * crop_y);
...
bool HeuristicOdometry::DumpBackground(std::string name, std::string path) {
  try {
    ...
    return cv::imwrite(dirPath.string() + "/" + name + ".jpg", currBackground,
                       compressionParams);
...
void HeuristicOdometry::SaveToVideo(cv::Mat &image) {
  ...
  if (!video.isOpened()) {
    // Open the video
    // video = cv::VideoWriter(outputVideoFile,
    // cv::VideoWriter::fourcc('M','J','P','G'), 60,
    // cv::Size(image.cols,image.rows));
    video = cv::VideoWriter(outputVideoFile,
                            cv::VideoWriter::fourcc('a', 'v', 'c', '1'), 60,
                            cv::Size(image.cols, image.rows));
...
void HeuristicOdometry::DumpRobotTrackerInfo(cv::Mat &channel,
                                             std::string title) {
  ...
  for (auto currRobot : _allRobotTrackers) {
    ...
    if ((bbox.x < 0) || (bbox.x > channel.rows - 1) || (bbox.y < 0) ||
        (bbox.y > channel.cols - 1)) {
      continue;
    }

    // Correct box size
    bbox.width -= (bbox.x + bbox.width >= channel.rows)
                      ? bbox.x + bbox.width - channel.rows
                      : 0;
    bbox.height -= (bbox.y + bbox.height >= channel.cols)
                       ? bbox.y + bbox.height - channel.cols
                       : 0;
    ...
    //
    // printText("Loc =" + std::to_string(currRobot->position.x) + "," +
    // std::to_string(currRobot->position.y), channel, 12 * i++, x_pos);
    // printText("Rot =" + std::to_string(currRobot->rotation.angle()), channel,
    // 12 * i++, x_pos); printText("Conf =" +
    // std::to_string(currRobot->delta_confidence), channel, 12 * i++, x_pos);
    // printText("Delta Loc = (" + std::to_string(currRobot->delta.x) + "," +
    // std::to_string(currRobot->delta.y) + ")", channel, 12 * i++, x_pos);
    // printText("Delta Rot =" + std::to_string(currRobot->delta_angle),
    // channel, 12 * i++, x_pos); printText(currRobot->debugLine, channel, 12 *
    // i++, x_pos);
    ...
    if (!currRobot->matchResultSaved.empty()) {
      cv::Rect rectToOutput(cv::Point2i(robotid * 200, 0),
                            currRobot->matchResultSaved.size());

      cv::Mat outputMatches;
      cv::normalize(currRobot->matchResultSaved, outputMatches, 0, 1.0,
                    cv::NORM_MINMAX);
      ...
      if (currRobot->finalCorrectionROI.area() > 0) {
        cv::Mat deratingMat;
        currRobot->distanceDerating(currRobot->finalCorrectionROI)
            .copyTo(deratingMat);

        deratingMat.convertTo(deratingMat, CV_8U, 255.0);
        cv::Rect rect2ToOutput(cv::Point2i(robotid * 200, 200),
                               deratingMat.size());
...
void HeuristicOdometry::DumpCurrFrameInfo(cv::Mat &channel,
                                          cv::Mat &currForeground,
                                          std::string title) {
  printText(title, channel, 60);
...
//**************** What are the implications though? These shouldn't live very
//long. What about compute power?
...
bool HeuristicOdometry::LocateRobots(cv::Point2f newPos, bool opponentRobot,
  ...
  if (robotTracker != nullptr) {
    ...
    _allRobotTrackers.erase(std::remove(_allRobotTrackers.begin(),
                                        _allRobotTrackers.end(), robotTracker),
                            _allRobotTrackers.end());
  ...
  int index1 = -1;
  float distance = FindClosestValidTrackedBox(tracked_bboxes, newPos, bestBBox,
                                              index1, true);

  int maxDimension =
      (opponentRobot) ? MAX_OPPONENT_BLOB_SIZE : MAX_ROBOT_BLOB_SIZE;
...
bool HeuristicOdometry::LocateRobots(cv::Point2f newPos, bool opponentRobot)
{
    ...
    RobotTracker *&robotTracker = (opponentRobot) ? opponentRobotTracker :
ourRobotTracker; RobotTracker *&otherTracker = (opponentRobot) ? ourRobotTracker
: opponentRobotTracker;
    ...
    // If we found a tracker of sufficient size that is reasonably close that
has cleared start tracking if (closest != nullptr && closestDistance <
max_distance_to_locate && closest->clearedStart)
    {
...
// void HeuristicOdometry::DrawAllBBoxes(cv::Mat &mat, int thickness, cv::Scalar
// scaler)
//{
//     for (const auto &bbox : all_bboxes)
//     {
//         cv::rectangle(mat, bbox, scaler, thickness, cv::LINE_4);
//         printText(std::to_string(bbox.width) + "x" +
//         std::to_string(bbox.height), mat, bbox.y, bbox.x);
//     }
// }

void HeuristicOdometry::DrawAllBBoxes(cv::Mat &mat, int thickness,
                                      cv::Scalar scaler) {
  // Define thickness for tracked and untracked boxes
  int tracked_thickness = thickness + 1;  // Thicker frames for tracked boxes
  int untracked_thickness =
      thickness;  // Original thickness for untracked boxes

  // Ensure scaler is suitable for 8-bit grayscale (single channel)
  cv::Scalar tracked_scaler(200);  // Bright gray for tracked boxes
  cv::Scalar untracked_scaler(
      scaler[0]);  // Use input scaler value for untracked boxes

  // Draw tracked_bboxes with detailed information and thicker frames
  for (const auto &tracked : tracked_bboxes) {
    if (!tracked.is_active) continue;

    // Draw tracked box with thicker frame
    cv::rectangle(mat, tracked.bbox, tracked_scaler, tracked_thickness,
                  cv::LINE_4);
  ...
  for (const auto &tracked : tracked_bboxes) {
    if (tracked.is_active) tracked_rects.push_back(tracked.bbox);
  }
  ...
  for (const auto &bbox : all_bboxes) {
    ...
    if (!is_tracked) {
      cv::rectangle(mat, bbox, untracked_scaler, untracked_thickness,
                    cv::LINE_4);
  ...
  cv::Rect leftrect(STARTING_LEFT_TL_x, STARTING_LEFT_TL_y,
                    (STARTING_LEFT_BR_x - STARTING_LEFT_TL_x),
                    (STARTING_LEFT_BR_y - STARTING_LEFT_TL_y));
  cv::Rect rightrect(STARTING_RIGHT_TL_x, STARTING_RIGHT_TL_y,
                     (STARTING_RIGHT_BR_x - STARTING_RIGHT_TL_x),
                     (STARTING_RIGHT_BR_y - STARTING_RIGHT_TL_y));
...
std::string getImageType(int type) {
  ...
  switch (depth) {
    case CV_8U:
      r = "8-bit unsigned (CV_8U)";
      break;
    case CV_8S:
      r = "8-bit signed (CV_8S)";
      break;
    case CV_16U:
      r = "16-bit unsigned (CV_16U)";
      break;
    case CV_16S:
      r = "16-bit signed (CV_16S)";
      break;
    case CV_32S:
      r = "32-bit signed (CV_32S)";
      break;
    case CV_32F:
      r = "32-bit float (CV_32F)";
      break;
    case CV_64F:
      r = "64-bit float (CV_64F)";
      break;
    default:
      r = "Unknown type";
      break;
...
void HeuristicOdometry::ExtractForeground(cv::Mat &croppedFrame) {
  ...
  // First get the absolute difference (leaving >0 values where there is a
  // difference)
  ...
  cv::threshold(foregroundBlurred, fg_mask, fg_threshold, 255,
                cv::THRESH_BINARY);
  ...
  // But the number it finds is way to large, thus we want to get rid of
  // speckles and only look at bigger contours Do do this, lets blur the image,
  // re-threshold, down-sample, use that for bbox discovery then lets up-sample
  // it

  // First lets blur the mask. This double blur seems to work well (balances how
  // much backround to include versus filling in empty spots)
  cv::Mat fg_mask_blurred;
  cv::blur(fg_mask, fg_mask_blurred, fg_mask_blur_size);
  cv::threshold(fg_mask_blurred, fg_mask, fg_post_blur_threshold, 255,
                cv::THRESH_BINARY);
  // cv::blur(fg_mask, fg_mask_blurred, fg_mask_blur_size);
  // cv::threshold(fg_mask_blurred, fg_mask, fg_post_blur_threshold, 255,
  // cv::THRESH_BINARY);
  ...
  // cv::findContours(fg_mask_small, contours, cv::RETR_EXTERNAL,
  // cv::CHAIN_APPROX_SIMPLE); cv::findContours(fg_mask_small, contours,
  // cv::RETR_EXTERNAL, cv::CHAIN_APPROX_TC89_L1);
  cv::findContours(
      fg_mask_small, contours, cv::RETR_EXTERNAL,
      cv::CHAIN_APPROX_TC89_KCOS);  // Seems to have less points then Simple

  // Lets fill all contours
  for (size_t i = 0; i < contours.size(); i++) {
    cv::drawContours(fg_mask_small, contours, (int)i, cv::Scalar(255),
                     cv::FILLED);
  }
    ...
    for (size_t i = 0; i < all_bboxes.size(); ++i) {
      ...
      for (size_t j = i + 1; j < all_bboxes.size(); ++j) {
        ...
        if ((all_bboxes[i] & all_bboxes[j]).area() > 0) {
          ...
          all_bboxes[i] |= all_bboxes[j];  // Set bboxes[i] to the union of
                                           // bboxes[i] and bboxes[j]
  ...
  for (int i = all_bboxes.size() - 1; i >= 0; i--) {
    if (!erased_bbox[i]) {
      all_boxes_clean.push_back(
          myRect(FixBBox(all_bboxes[i], foreground.size())));
    }
  }
  ...
  // Is this really needed? Looks like the sparkles and noise risks being added
  // to the tracked image, thus we do want to remove all noise
  ...
  for (size_t i = 0; i < all_bboxes.size(); ++i) {
    for (size_t j = 0; j < tracked_bboxes.size(); ++j) {
      ...
      if (CalculateIoU(all_bboxes[i], tracked_bboxes[j].bbox) > iou_threshold &&
          IsSimilarSize(all_bboxes[i], tracked_bboxes[j].bbox,
                        size_threshold)) {
        ...
        tracked_bboxes[j].velocity = cv::Point2f(new_center.x - old_center.x,
        ...
        if (!tracked_bboxes[j].is_clear &&
            (tracked_bboxes[j].bbox * tracked_bboxes[j].startBBox).area() <=
                0) {
  ...
  for (size_t i = 0; i < all_bboxes.size(); ++i) {
    if (!matched_new_bboxes[i]) {
      ...
      // If we're in the starting lock period, make the new_tracked bbox be only
      // the starting rectangles
      if (heuStateMachine == CMSM_START_WAIT_ROBOT_CLEAR) {
  ...
  matched_tracked_bboxes.resize(tracked_bboxes.size(),
                                true);  // New boxes are matched and active
  ...
  tracked_bboxes.erase(
      std::remove_if(tracked_bboxes.begin(), tracked_bboxes.end(),
                     [](const TrackedBBox &tb) {
                       return !tb.is_active && tb.frame_count > 4;
                     }),
      tracked_bboxes.end());
}

// Heal the background by slowly averaging areas where there is no robot lock
// We will average in new frames to the background to get the average background
// image Ideally we don't want to average in activelly tracked robots, however
// we need a mechanism to remove invalid areas that are tracked. We will thus
// implement a very slow decay for tracked areas, and a much faster decay for
// untracked areas, relying on the fact that a robot should move relativelly
// quickly.
void HeuristicOdometry::healBackground(cv::Mat &currFrame,
  ...
  cv::Mat newBackground_16bit =
      currBackground_16bit * ((myAveragingCount - 1.0) / myAveragingCount) +
      currFrame_16bit * 1.0 / myAveragingCount;

  // Slow tracked averaging
  cv::Mat newBackground_16bit_slow =
      currBackground_16bit * ((myTrackedAvgCount - 1.0) / myTrackedAvgCount) +
      currFrame_16bit * 1.0 / myTrackedAvgCount;

  // Robot locks get the slowest averaging
  // cv::Mat newBackground_16bit_ultraslow =
  // currBackground_16bit*((robotAvgCount-1.0)/robotAvgCount) +
  // currFrame_16bit*1.0/robotAvgCount;
  ...
  for (const auto &bbox : all_bboxes) {
    ...
    if (fixedBBox.area() > 0) {
      newBackground_16bit_slow(fixedBBox).copyTo(
          newBackground_16bit(fixedBBox));
    }
  }
...
void HeuristicOdometry::MatchStartBackgroundInit(cv::Mat &currFrame) {
  ...
  cv::Mat leftsquare =
      regularBackground(leftrect).clone() * leftAdjust * HEU_BRIGHTNESS_CORR;
  cv::Mat rightsquare =
      regularBackground(rightrect).clone() * rightAdjust * HEU_BRIGHTNESS_CORR;
...
float HeuristicOdometry::GetForegroundSize(cv::Mat &inBackground,
                                           cv::Mat &inForeground) {
  // First get mask for foreground extraction
  cv::Mat currBackgroundBlurred;
  cv::Mat croppedFrameBlurred;
  ...
  cv::Mat myFgMask;
  // First get the mask that includes regions at least the minimum pixels away
  cv::threshold(foregroundBlurred, myFgMask, fg_threshold, 255,
                cv::THRESH_BINARY);
...
float HeuristicOdometry::FindOptimalBrightness(cv::Rect &myRect,
                                               cv::Mat &currFrame) {
  ...
  for (float brightnessAdjust = 0.3f; brightnessAdjust < 3.0f;
       brightnessAdjust *= 1.1f) {
...
bool HeuristicOdometry::IsClearOfArea(RobotTracker *tracker,
                                      const cv::Rect &area) {
  if (!tracker) return false;

  // Expand the area by the clearance margin
  cv::Rect expanded_area(area.x - clear_margin, area.y - clear_margin,
                         area.width + 2 * clear_margin,
                         area.height + 2 * clear_margin);
...
// Returns the brightness correction factor for the specified square in the
// image 1.0f means no correction
float HeuristicOdometry::correctBrightness(cv::Mat &image,
  ...
  if (!reset_averaging && (IMAGE_INTENSITY_TIME_CONSTANT < 0) ||
      ...
      refBackground.empty() || currBackground.empty() ||
      currBackground.cols < IMAGE_INTENSITY_SQR_size ||
      image.cols < IMAGE_INTENSITY_SQR_size) {
    refIntensities[0] = 0;
    refIntensities[1] = 0;
    refIntensities[2] = 0;
    refIntensities[3] = 0;
    brightnessCorrection = 1.0f;
    return 1.0f;  // Safer to return 1.0f so that the state machine doesnt lock
                  // up
  }

  // If we haven't calculated our reference background intensities yet, then do
  // that now
  if ((refIntensities[0] <= 0) || reset_averaging) {
    // refBackground assumed to be black and white (1 channel)
    refIntensities[0] = cv::mean(refBackground(
        cv::Rect(IMAGE_INTENSITY_SQR_1_x, IMAGE_INTENSITY_SQR_1_y,
                 IMAGE_INTENSITY_SQR_size, IMAGE_INTENSITY_SQR_size)))[0];
    refIntensities[1] = cv::mean(refBackground(
        cv::Rect(IMAGE_INTENSITY_SQR_2_x, IMAGE_INTENSITY_SQR_2_y,
                 IMAGE_INTENSITY_SQR_size, IMAGE_INTENSITY_SQR_size)))[0];
    refIntensities[2] = cv::mean(refBackground(
        cv::Rect(IMAGE_INTENSITY_SQR_3_x, IMAGE_INTENSITY_SQR_3_y,
                 IMAGE_INTENSITY_SQR_size, IMAGE_INTENSITY_SQR_size)))[0];
    refIntensities[3] = cv::mean(refBackground(
        cv::Rect(IMAGE_INTENSITY_SQR_4_x, IMAGE_INTENSITY_SQR_4_y,
                 IMAGE_INTENSITY_SQR_size, IMAGE_INTENSITY_SQR_size)))[0];
  }
  ...
  float scaler = 0.0;
  if (!reset_averaging &&
      (deltaTime < IMAGE_INTENSITY_TIME_CONSTANT && deltaTime > 0)) {
  ...
  corrections[0] = getBrightnessCorrection(
      image, IMAGE_INTENSITY_SQR_1_x, IMAGE_INTENSITY_SQR_1_y,
      IMAGE_INTENSITY_SQR_size, refIntensities[0]);
  corrections[1] = getBrightnessCorrection(
      image, IMAGE_INTENSITY_SQR_2_x, IMAGE_INTENSITY_SQR_2_y,
      IMAGE_INTENSITY_SQR_size, refIntensities[1]);
  corrections[2] = getBrightnessCorrection(
      image, IMAGE_INTENSITY_SQR_3_x, IMAGE_INTENSITY_SQR_3_y,
      IMAGE_INTENSITY_SQR_size, refIntensities[2]);
  corrections[3] = getBrightnessCorrection(
      image, IMAGE_INTENSITY_SQR_4_x, IMAGE_INTENSITY_SQR_4_y,
      IMAGE_INTENSITY_SQR_size, refIntensities[3]);
  ...
  brightnessCorrection =
      brightnessCorrection * scaler + (1.0 - scaler) * newcorrection;

  // Now adjust the new image
  image.convertTo(image, CV_16U, brightnessCorrection,
                  0);  // Multiply by factor, no offset
...
void HeuristicOdometry::calcShakeRemoval(cv::Mat &image) {
  ...
  cv::Rect cropparea(x_start_region, y_start_region, antiShakeSize,
                     antiShakeSize);
  cv::Mat cropBgToFind = currBackground(cropparea);

  // Now crop the image for the region to scan
  cv::Rect cropimg(x_start_region, y_start_region - crop_y,
                   antiShakeSize + 2 * crop_x, antiShakeSize + 2 * crop_y);
  ...
  if ((abs(maxLoc.y - crop_y) <= max_shake_correction) &&
      (abs(maxLoc.x - crop_x) <= max_shake_correction)) {
    y_offset = maxLoc.y;
    x_offset = maxLoc.x;
  }
}
...
// Returns true if width and height difference is less then max_size_error
// fraction.
bool HeuristicOdometry::IsSimilarSize(const myRect &box1, const myRect &box2,
                                      float max_size_error) {
  float width_ratio =
      std::abs(box1.width - box2.width) / (std::max)(box1.width, box2.width);
  float height_ratio = std::abs(box1.height - box2.height) /
