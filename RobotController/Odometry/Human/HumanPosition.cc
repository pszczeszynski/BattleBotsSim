...
HumanPosition::HumanPosition(ICameraReceiver* videoSource, std::string port,
                             bool sendHeuristicMat)
...
void HumanPosition::_ProcessNewFrame(cv::Mat currFrame, double frameTime) {
  ...
  if (_sendHeuristicMat) {
    cv::Mat& heuristicFrame = RobotController::GetInstance()
                                  .odometry.GetHeuristicOdometry()
                                  .GetPrevTrackingMatRef();
  ...
  // Scale down image
  cv::Mat scaled;
  cv::resize(frameToUse, scaled,
             cv::Size(WIDTH / IMG_SCALE, HEIGHT / IMG_SCALE), 0, 0,
             cv::INTER_LINEAR);
  ...
  message += std::to_string(scaled_robotpos.x) + MSG_FIELD_SEPERATOR +
             std::to_string(scaled_robotpos.y) + MSG_FIELD_SEPERATOR;
  numOfFields += 2;
  ...
  message += std::to_string(scaled_opponentpos.x) + MSG_FIELD_SEPERATOR +
             std::to_string(scaled_opponentpos.y) + MSG_FIELD_SEPERATOR;
  numOfFields += 2;
  ...
  message +=
      std::to_string((int)(opponentAngle * 180 / M_PI)) + MSG_FIELD_SEPERATOR;
  numOfFields++;
  ...
  if (password_ok) {
    ...
    // *************************************
    // Do button presses
    HeuristicOdometry& heuristic =
        RobotController::GetInstance().odometry.GetHeuristicOdometry();
    ...
    BlobDetection& blob =
        RobotController::GetInstance().odometry.GetBlobOdometry();

    if (type == DataType::ROBOT_POSITION) {
      // blob.SetPosition(clickPosition, false);
      if (force_pos_bool) {
        odometry.UpdateForceSetPosAndVel(clickPosition, cv::Point2f(0, 0),
                                         false);
      ...
      Angle angle =
          Angle(atan2(clickPosition.y - MIDDLE, clickPosition.x - MIDDLE));
