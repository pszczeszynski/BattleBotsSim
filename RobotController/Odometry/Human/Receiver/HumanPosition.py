...
def parse_server_data(
    ...
    curr_pos = 0
    try:
        # Find the end of the text data. Assuming '%' is the last separator
        end_text = data.index(b'$')  # Find our binary data seperator
        ...
        # status_label.set("Parts = " + str(len(parts)))
        ...
        if (time.time() - lastSliderTime) > 0.5:           
            ...
            bg_heal_rate.set(str(bg_heal_rate_int))
            background_heal_rate_slider.set(bg_heal_rate_int)
            force_heal_var.set(int(parts[index]))
        ...
        # status_label.set("Image Size = " + str(image_size) + " Image Len = " + str(len(image_data) ))
...
currRow = 0
# status_line = tk.Label(side_panel, textvariable=status_label)
# status_line.grid(row=currRow, column=0, columnspan=2,  padx=5, pady=5)
# currRow+=1
...
#mode_label.grid(row=currRow, column=0, columnspan=2, padx=5, pady=5)
#currRow+=1
