...
constexpr int IMGUI_WIDTH = 1;
constexpr int IMGUI_HEIGHT = 1;
...
bool RobotControllerGUI::Update() {
  ...
  // draw all the clock widgets (including Profiling) FIRST - this makes them
  // secondary tabs
  ClockWidget::DrawAll();

  // draw all the image widgets (including Orbit) SECOND - this makes them
  // primary tabs
...
GLFWwindow *RobotControllerGUI::SetupWindow() {
  ...
  GLFWwindow *window = glfwCreateWindow(IMGUI_WIDTH, IMGUI_HEIGHT,
                                        "RobotController", nullptr, nullptr);
  if (window == nullptr) {
    // return failure
    return nullptr;
  }
...
void RobotControllerGUI::SetupIMGUI(GLFWwindow *window) {
  ...
  ImFont *customFont =
      io.Fonts->AddFontFromFileTTF("C:\\Windows\\Fonts\\consolab.ttf", 15.0f);
  // terminal
